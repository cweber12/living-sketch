export const PersonArmsUpIcon = ({
  size = '20px',
  color = 'currentColor',
}: {
  size?: string | number;
  color?: string;
}) => (
  <svg
    fill={color}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M9.5,3.5A2.5,2.5,0,1,1,12,6,2.5,2.5,0,0,1,9.5,3.5ZM3,8A1,1,0,0,1,4,7H20a1,1,0,0,1,0,2H15V22a1,1,0,0,1-2,0V15H11v7a1,1,0,0,1-2,0V9H4A1,1,0,0,1,3,8Zm8,5h2V9H11Z" />
  </svg>
);

export const PersonArmsDownIcon = ({
  size = '20px',
  color = 'currentColor',
}: {
  size?: string | number;
  color?: string;
}) => (
  <svg
    fill={color}
    width={size}
    height={size}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M6.684,14.949a1,1,0,0,0,1.265-.633L9,11.162V22a1,1,0,0,0,2,0V16h2v6a1,1,0,0,0,2,0V11.162l1.052,3.154a1,1,0,1,0,1.9-.632l-2-6A1,1,0,0,0,15,7H9a1,1,0,0,0-.949.684l-2,6A1,1,0,0,0,6.684,14.949ZM11,9h2v5H11ZM9.5,3.5A2.5,2.5,0,1,1,12,6,2.5,2.5,0,0,1,9.5,3.5Z" />
  </svg>
);
